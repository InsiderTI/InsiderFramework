...
/**
  Classe de annotations do módulo de roteamento
 */
                ...
                switch ($declaration){
                    ...
                    case 'route':
                        if (isset($annotationData[$id]['route'])){
                            \KeyClass\Error::i10nErrorRegister("The %" . "@Route" . "% statement is duplicated in the class declaration in the controller %" . $id . "%", 'pack/sys');
                        }
                        
                        // Se o roteamento está como case sensitive
                        if ($routingSettings['routeCaseSensitive']){
                            // Buscando declaração de annotation com case sensitive
                            preg_match_all(read::$patternArgs, $commentsWithCase[$commentK], $annotationMatchesWithCase, PREG_SET_ORDER, 0);
                            
                            // Buscando os argumentos no comentário
                            preg_match_all(read::$betweenCommasPattern, $annotationMatchesWithCase[0]['args'], $argsCase, PREG_SET_ORDER, 0);
                        }

                        // Buscando os argumentos no comentário
                        preg_match_all(read::$betweenCommasPattern, $annotationMatches[0]['args'], $args, PREG_SET_ORDER, 0);
                        ...
                        foreach($args as $argK => $arg){
                            ...
                            if (isset($annotationData[$id]['route'][$argument])){
                                \KeyClass\Error::i10nErrorRegister("The %" . "@".$argument . "% argument is duplicated in statement ". $declaration ." in the controller %" . $id . "%", 'pack/sys');
                            }
                    ...
                    case 'permission':
                        if (isset($annotationData[$id]['permission'])){
                            \KeyClass\Error::i10nErrorRegister("The %" . "@Permission" . "% statement is duplicated in the class declaration in the controller %" . $id . "%", 'pack/sys');
                        }
                        ...
                        preg_match_all(read::$betweenCommasPattern, $annotationMatchesWithCase[0]['args'], $argsWithCase, PREG_SET_ORDER, 0);
                        preg_match_all(read::$betweenCommasPattern, $annotationMatches[0]['args'], $args, PREG_SET_ORDER, 0);
                    ...
                    case 'param':
                        ...
                        foreach($paramRoute as $pR){
                            // Buscando declaração de annotation
                            preg_match_all(read::$betweenCommasPattern, $pR, $pRMatches, PREG_SET_ORDER, 0);
                    ...
                    case "verbs":
                        if (isset($annotationData[$id]['verbs'])){
                            \KeyClass\Error::i10nErrorRegister("The %" . "@Verbs" . "% statement is duplicated in the class declaration in the controller %" . $id . "%", 'pack/sys');
                        }
                    ...
                    case "cache":
                        if (isset($annotationData[$id]['cache'])){
                            \KeyClass\Error::i10nErrorRegister("The %" . "@Cache" . "% statement is duplicated in the class declaration in the controller %" . $id . "%", 'pack/sys');
                        }
                        $verbsRoute = explode(',',$annotationMatches[0]['args']);
                    ...
                    case "domains":
                        if (isset($annotationData[$id]['domains'])){
                            \KeyClass\Error::i10nErrorRegister("The %" . "@Domains" . "% statement is duplicated in the class declaration in the controller %" . $id . "%", 'pack/sys');
                        }
                    ...
                    case "responseformat":
                        if (isset($annotationData[$id]['responseformat'])){
                            \KeyClass\Error::i10nErrorRegister("The %" . "@Responseformat" . "% statement is duplicated in the class declaration in the controller %" . $id . "%", 'pack/sys');
                        }
